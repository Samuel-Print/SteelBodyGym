...
const EditClassModal = ({ open, onClose, classData, onSave }) => {
  ...
  const [removedHorarioIds, setRemovedHorarioIds] = useState([]);
  ...
  useEffect(() => {
    if (classData) {
      setFormData({
        ...
        imagen: null,
        imagen_preview: classData.imagen_url || null,
      });

      const existentes = (classData.horarios || []).map((h) => ({
        tempId: crypto.randomUUID(),
        id_horario: h.id_horario,
        dia_semana: h.dia_semana,
        hora_inicio: h.hora_inicio?.slice(0, 5) || "06:00",
        hora_fin: h.hora_fin?.slice(0, 5) || "07:00",
      }));

      setHorarios(existentes.length ? existentes : [newHorarioRow()]);
      setRemovedHorarioIds([]);
    } else {
      setFormData({ nombre: "", descripcion: "", imagen: null, imagen_preview: null });
  ...
  const removeHorario = (tempId) => {
    const target = horarios.find((h) => h.tempId === tempId);
    if (target?.id_horario) {
      setRemovedHorarioIds((prev) => [...prev, target.id_horario]);
    }
    ...
    try {
      await toast.promise(
        Promise.resolve(onSave(dataToSend)),
        {
          ...
          error: "No se pudo actualizar la clase. Intenta de nuevo.",
        },
        {
  ...
  return (
    <Modal isOpen={open} onClose={handleClose}>
      ...
      >
        <h3 className="text-[18px] font-head font-bold text-[var(--text)]">Editar clase</h3>
        <p className="text-[13px] text-[var(--muted)]">Modifica la información de la clase.</p>
      </ModalHeader>
